...
import { db, type Module, type DependencyElement } from '../db/butlerDB';
...
export default function ModuleDetails() {
    ...
    // --- DEPENDENCY LOGIC ---
    // Fetch detailed public elements from other modules for the modal
    ...
    const loadDependencyData = async () => {
        ...
        const data = await Promise.all(siblings.map(async (mod) => {
            ...
            return {
                module: mod,
                elements: [
                    ...ents.filter(e => e.isPublic).map(e => ({ id: e.id, name: e.name, type: 'Entity' })),
                    ...acts.filter(a => a.isPublic).map(a => ({ id: a.id, name: a.name, type: 'Action' })),
                    ...uis.filter(u => u.isPublic).map(u => ({ id: u.id, name: u.name, type: 'UI' }))
                ]
            };
        }));
    ...
    const handleDeleteUI = async (id: string) => {
        if (confirm('Delete this UI Element?')) await db.uiElements.delete(id);
    };

    const togglePublicUI = async (ui: any) => {
        await db.uiElements.update(ui.id, { isPublic: !ui.isPublic });
    };

    // --- EXISTING HANDLERS ---
    const handleCopyContext = () => {
        if (!module) return;
        const jsonContext = generateModuleContext(module, entities || [], actions || [], uiElements || [], projectModules || []);
        navigator.clipboard.writeText(jsonContext);
        alert("Context copied to clipboard!");
    };

    const handleExportXML = () => { /* XML Logic (Kept Simple for now) */ alert("XML Export for Screens coming soon. JSON Backup includes them."); };
    const processXML = async (xml: string) => { /* Reuse existing logic */ };
        ...
        <div className="min-h-screen bg-[#F3F4F6] pb-20">
            ...
            <div className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
                <div className="max-w-[1400px] mx-auto px-6 h-20 flex justify-between items-center">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                ...
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${module.layer === 'End-User' ? 'bg-green-50 text-green-700' : module.layer === 'Core' ? 'bg-yellow-50 text-yellow-700' : 'bg-blue-50 text-blue-700'}`}>
                                    {module.layer}
                                </span>
                            </div>
                            ...
                            {isEditingDesc ? (
                                <div className="flex gap-2 mt-1">
                                    <input
                                        className="border rounded px-2 py-0.5 text-xs w-96 outline-none focus:ring-1 focus:ring-blue-500"
                                        value={tempDesc}
                                ...
                                <div className="group flex items-center gap-2 mt-1 cursor-pointer" onClick={() => setIsEditingDesc(true)}>
                                    <p className="text-xs text-gray-500 italic hover:text-gray-700 transition">
                                        {module.description || "Click to add a description..."}
                                    </p>
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={() => { loadDependencyData(); setShowDependenciesModal(true); }} className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg text-xs font-bold hover:bg-gray-50 transition shadow-sm flex items-center gap-2">
                            <span>🔗</span> Dependencies
                        </button>
                        <div className="h-8 w-px bg-gray-200 mx-1"></div>
                        <button onClick={handleCopyContext} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-indigo-700 transition shadow-md flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col h-[650px]">
                        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-xl">
                            ...
                            <div className="flex gap-1">
                                <button onClick={() => handleAddUI('Screen')} className="text-green-600 text-xs font-bold hover:bg-green-50 px-2 py-1 rounded transition">+ Screen</button>
                                <button onClick={() => handleAddUI('Block')} className="text-green-600 text-xs font-bold hover:bg-green-50 px-2 py-1 rounded transition">+ Block</button>
                            </div>
                        </div>
                        <div className="p-3 overflow-y-auto flex-grow space-y-2">
                            {uiElements?.map(ui => (
                                <div key={ui.id} className="group p-3 border border-gray-100 rounded-lg hover:border-green-400 hover:shadow-sm transition bg-white flex justify-between items-center">
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <span className="font-bold text-sm text-gray-700">{ui.name}</span>
                                            <span className="text-[9px] uppercase text-gray-400 border px-1 rounded">{ui.type}</span>
                                        </div>
                                        <div className="flex items-center gap-2 mt-1">
                                            <label className="text-[10px] flex items-center gap-1 cursor-pointer text-gray-500 hover:text-green-600">
                            ...
                            {uiElements?.length === 0 && (
                                <div className="h-full flex flex-col items-center justify-center text-gray-400">
                                    ...
                                    <p className="text-xs">No Screens or Blocks</p>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {/* --- MODAL: GRANULAR DEPENDENCY MANAGER --- */}
                        ...
                        <div className="flex flex-grow overflow-hidden">
                                ...
                                {availablePublicElements.map((prod) => (
                                    <div key={prod.module.id} className="mb-6">
                                        <div className="font-bold text-sm text-gray-800 mb-2 flex items-center gap-2">
                                            {prod.module.name}
                                            <span className="text-[9px] bg-gray-200 px-1 rounded font-normal">{prod.module.layer}</span>
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            {prod.elements.map((el: any) => {
                                                ...
                                                return (
                                                    <div
                                                        key={el.id}
                                                        onClick={() => toggleDependencyElement(prod.module.id, el)}
                                                        className={`flex items-center gap-2 p-2 rounded cursor-pointer text-xs border transition ${isSelected ? 'bg-blue-50 border-blue-300 text-blue-800' : 'bg-white border-gray-200 hover:bg-gray-100'}`}
                                                    >
                                                        ...
                                                        <span className="ml-auto text-[9px] text-gray-400 uppercase">{el.type}</span>
                                                    </div>
                                                )
                                            })}
                                        </div>
                            ...
                            <div className="w-2/3 p-8 flex flex-col items-center justify-center text-center text-gray-500">
                                ...
                                <p className="max-w-md">
                                    Select public elements from the left to consume them in <b>{module.name}</b>.
                                    <br /><br />
                                    These dependencies will be visible to the AI, allowing it to understand relationships between your modules.
                                </p>
                            </div>
                        </div>

                        <div className="p-4 border-t bg-gray-50 flex justify-end">
                            <button onClick={() => setShowDependenciesModal(false)} className="bg-gray-900 text-white px-6 py-2 rounded-lg font-bold hover:bg-black transition shadow-md">
                                Done
                            </button>
                        </div>
                    </div>
